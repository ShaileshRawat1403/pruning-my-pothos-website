...
heroImage: '/covers/systems/drift-decay-and-silent-failure.svg'
heroImageAlt: 'Degradation curve showing performance decline over time'
tags:
  - drift
  - monitoring
  - reliability
...
faq:
  - question: What is drift in AI systems?
    answer: Drift is gradual behavior change over time, not a single defect. It occurs when the data distribution, user intent, or model behavior shifts subtly. Unlike bugs that cause immediate failure, drift causes slow degradation that may go unnoticed.
  - question: What is silent failure in AI?
    answer: Silent failure is when outputs remain mostly correct in form but diverge in meaning or accuracy. The system appears to work ("it's generating responses") but the outputs no longer meet requirements. This is harder to detect than loud failures.
  - question: How do you detect drift?
    answer: Drift detection requires monitoring intent alignment, not just technical metrics. Track evaluation scores over time, compare outputs against known-good baselines, monitor user feedback signals, and set thresholds that trigger review when behavior shifts beyond acceptable bounds.
---
...
### What causes drift in production systems?

Drift occurs when the data distribution, user behavior, or model response patterns shift over time, leading to gradual behavior change. Because the system's outputs remain structurally valid, these shifts do not trigger typical software exception logs, resulting in silent performance decay rather than a loud application crash.
...
<figure class="diagram">
  <iframe src="/scenes/drift-decay.html" title="Data Drift Leading to Performance Decay" loading="lazy" scrolling="no" style="pointer-events: none;"></iframe>
...
For related systems context, see [Systems 001: Foundations](/systems/systems-001-foundations/) and [From Prompt to Production](/systems/from-prompt-to-production/). For practical tools to evaluate intent alignment and prevent decay, see the [retrieval and grounding evaluation kit](/shelf/shared-resources/retrieval-and-grounding-evaluation-kit/) in our shared resources library.
