...
heroImageAlt: 'Human judgment vs benchmarks'
tags:
  - evaluation
  - quality
  - judgment
...
faq:
  - question: Why are benchmarks not enough for AI evaluation?
    answer: Benchmarks measure performance on standardized tests, but they don't tell you if a model is good for your specific purpose. Human judgment defines what "good" means for your use case. A model can score well on benchmarks but fail at your task.
  - question: What is a rubric in AI evaluation?
    answer: A rubric is a structured scoring guide that encodes human judgment into repeatable criteria. It defines what good looks like across multiple dimensions (accuracy, safety, helpfulness) so different evaluators can score consistently.
  - question: Why is evaluation a loop, not a score?
    answer: AI systems change behavior as prompts, context, and models evolve. A single evaluation score becomes stale quickly. Continuous evaluation with regular sampling, rubric refinement, and trend tracking captures degradation before it becomes a production problem.
---
...
### Why is evaluation a human problem?

Evaluation is a human problem because quality is contextual: benchmarks measure general capabilities, but human judgment defines what "correctness" means for a specific task. To build reliable systems, we must translate subjective preferences into structured, repeatable rubrics that can evaluate outputs consistently over time.
...
<figure class="diagram">
  <iframe src="/scenes/eval-human-cycle.html" title="Human-in-the-Loop Evaluation Cycle" loading="lazy" scrolling="no" style="pointer-events: none;"></iframe>
...
For related systems context, see [Systems 001: Foundations](/systems/systems-001-foundations/) and [From Prompt to Production](/systems/from-prompt-to-production/). To see how rubrics and evaluation datasets are set up for validation, explore the [retrieval and grounding evaluation kit](/shelf/shared-resources/retrieval-and-grounding-evaluation-kit/).
