...
heroImageAlt: 'Neural network nodes showing pattern recognition and probability distribution'
tags:
  - ai
  - foundations
  - models
...
faq:
  - question: What is an AI model fundamentally?
    answer: An AI model is a statistical engine that predicts the next piece of text based on patterns learned during training. It does not "understand" in the human sense; it generates statistically probable continuations of input sequences.
  - question: Why do AI models sometimes give wrong answers confidently?
    answer: Models predict probable text, not correct text. When a statistically probable but factually wrong answer fits the pattern, the model produces it with the same confidence as correct answers. There is no internal "truth detector" in next-token prediction.
  - question: What are model weights?
    answer: Weights are the parameters learned during training. They encode statistical relationships between concepts, words, and patterns in the training data. A 70B parameter model has 70 billion of these numerical values that determine how inputs map to outputs.
---
...
<figure class="diagram">
  <iframe src="/scenes/training-vs-inference.html" title="Training vs Inference" loading="lazy" scrolling="no" style="pointer-events: none;"></iframe>
...
For related systems context, see [Systems 001: Foundations](/systems/systems-001-foundations/) and [From Prompt to Production](/systems/from-prompt-to-production/). For a compact companion on runtime behavior, see [Training, Fine-Tuning, and Inference](/systems/training-vs-inference/). To see how local models are deployed and tested as baseline inference engines, review our [Ollama launch integrations experiment](/shelf/local-experiments/ollama-launch-integrations/).
